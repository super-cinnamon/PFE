...
while(1):
	...
	if elapsed_time > seconds:
		...
		move.append(min(yaw))
		if((max(roll)-min(roll))<7 or (max(pitch)-min(pitch))<7 or (max(yaw)-min(yaw))<7 ):
			print("idle")
			ser.write(b'0')
			...
			testdataset = KNN.ClassifyI(Obj1, dataset[0], move, 6) #optimal value for k in tests was 7 (we wrote 6 because the class adds 1 so to get 7 neighbors we must put 6)
			# print the result
			print(testdataset)
			if(testdataset[0] == "on "):
				ser.write(b'1')
